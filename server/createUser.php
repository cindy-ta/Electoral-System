...
if(isset($_GET['email']) && isset($_GET['password']) && isset($_GET['user_type']))
{
    ...
    $query="Select max(user_name) from Users where user_type = '$user_type'";
    $result = $mysqli->query($query) or die($mysqli->error.__LINE__);

    if($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $user_name = $row['user_name'] + 1;
        }
    }
    ...
    $query="INSERT INTO Users (email,password,user_type,username,verification_code,verified)  VALUES ('$email', '$password', '$user_type', '$user_name', '$code', '$verified')";
    $result = $mysqli->query($query) or die($mysqli->error.__LINE__);
